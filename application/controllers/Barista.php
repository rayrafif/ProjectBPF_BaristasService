...
class Barista extends CI_Controller
{
	...
	function edit($id)
	{
		$data['judul'] = "Halaman Edit Barista";
		$data['barista'] = $this->Barista_model->getById($id);
		$data['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array();
		$data['shift'] = $this->Shift_model->get();
		$this->form_validation->set_rules('nama', 'Nama Barista', 'required', [
			'required' => 'Nama Barista Wajib di isi'
		]);
		...
		if ($this->form_validation->run() == false) {
			...
			$this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">Data Barista Berhasil Diubah!</div>');
			redirect('Barista');
		}
	}
	function tambah()
	{
		$data['judul'] = "Halaman Tambah Barista";
		$data['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array();
		$data['shift'] = $this->Shift_model->get();
		$this->form_validation->set_rules('nama', 'Nama Barista', 'required', [
			'required' => 'Nama Barista Wajib di isi'
		...
		if ($this->form_validation->run() == false) {
			...
			$this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">Data Barista Berhasil Ditambah!</div>');
			redirect('Barista');
		}
	}
