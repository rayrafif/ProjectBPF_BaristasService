...
class Coffee extends CI_Controller
{
	...
	function edit($id)
	{
		$data['judul'] = "Halaman Edit Coffee";
		...
		$this->form_validation->set_rules('barista', 'Barista', 'required', [
			'required' => 'Barista Wajib di isi'
		]);
		...
		if ($this->form_validation->run() == false) {
			...
			$this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">Data Coffee Berhasil Diubah!</div>');
			redirect('Coffee');
		}
	}
	function tambah()
	{
		$data['judul'] = "Halaman Tambah Coffee";
		$data['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array();
		$data['barista'] = $this->Barista_model->get();
		$this->form_validation->set_rules('nama', 'Nama Coffee', 'required', [
			'required' => 'Nama Coffee Wajib di isi'
		...
		if ($this->form_validation->run() == false) {
			...
			$this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">Data Coffee Berhasil Ditambah!</div>');
			redirect('Coffee');
		}
	}
	...
	public function hapus($id)
	{
		...
		$error = $this->db->error();
		if ($error['code'] != 0) {
			$this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert"><i class="icon fas fa-info-circle"></i>Data Coffee tidak dapat dihapus (sudah berelasi)!</div>');
		} else {
			$this->session->set_flashdata('message', '<div class="alert alert-success" role="alert"><i class="icon fas fa-check-circle"></i>Data Coffee Berhasil Dihapus!</div>');
		}
		redirect('Coffee');
	}
}
